...
kubectl create secret generic git-ssh -n workflows \
                --dry-run=client \
                --from-file=key=/Users/david.fox/.ssh/id_ed25519  -o yaml \
...
kubectl create secret generic git-known-hosts -n workflows \
                --dry-run=client \
                --from-file=ssh_known_hosts=/Users/david.fox/.ssh/known_hosts -o yaml \
                | kubeseal  --controller-namespace kubeseal \
                            --controller-name sealed-secrets \
...
kubectl create secret generic regcred -n workflows \
                --dry-run=client \
                --from-file=.dockerconfigjson=/Users/david.fox/.docker/config.json \
